<script lang="js">
  ...
  const clickOutside = (node, config = {}) => {
    const options = {
      include: [],
      onClickOutside: () => {},
      ...config,
   ...
   * @property {import("svelte").Snippet} children - The child elements of the date picker.
   * @property {any} [startDate=null] - Represents the start date for a date picker.
   * @property {any} [endDate=null] - Represents the end date for a date picker.
   * @property {string} [startDateTime="00:00"] - Represents the start time for the date picker (in HH:mm format).
   * @property {string} [endDateTime="00:00"] - Represents the end time for the date picker (in HH:mm format).
   * @property {Date} [today=new Date()] - Represents the current date.
   * @property {number} [defaultYear=today.getFullYear()] - Represents the default year for the date picker.
   * @property {number} [defaultMonth=today.getMonth()] - Represents the default month for the date picker.
   * @property {number} [startOfWeek=0] - Represents the start day of the week (0 for Sunday, 1 for Monday, etc.).
   * @property {boolean} [isMultipane=false] - Indicates whether the date picker has multiple panes.
   * @property {boolean} [isRange=false] - Indicates whether the date picker is in range mode.
   * @property {boolean} [isOpen=false] - Indicates whether the date picker is open.
   * @property {string} [align="left"] - Specifies the alignment of the date picker (e.g., 'left', 'center', 'right').
   * @property {string} [theme=""] - Represents the theme of the date picker.
   * @property {string[]} [disabledDates=[]] - An array of disabled dates.
   * @property {string[]} [enabledDates=[]] - An array of enabled dates.
   ...
   * @property {boolean} [alwaysShow=false] - Indicates whether the date picker should always be shown.
   * @property {boolean} [showYearControls=true] - Indicates whether year controls are displayed in the date picker.
   * @property {boolean} [showPresets=false] - Indicates whether preset options are displayed in the date picker.
   * @property {boolean} [showTimePicker=false] - Indicates whether the time picker is shown in the date picker.
   * @property {boolean} [enableFutureDates=false] - Indicates whether future dates are enabled.
   * @property {boolean} [enablePastDates=true] - Indicates whether past dates are enabled.
   * @property {string[]} [presetLabels=["Today", "Last 7 Days", "Last 30 Days", "Last 60 Days", "Last 90 Days", "Last Year"]] - An array of preset date range labels.
   * @property {string[]} [dowLabels=["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"]] - An array of day-of-week labels.
   * @property {string[]} [monthLabels=["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"]] - An array of month labels.
   * @property {boolean} [includeFont=true] - Indicates whether the default font "Rubik" should be loaded.
   * @property {number} [MILLISECONDS_IN_DAY=24 * 60 * 60 * 1000] - The number of milliseconds in a day.
   * @property {(days: number) => number} [getDateFromToday=(days) => Date.now() - days * MILLISECONDS_IN_DAY] - A function to calculate a date based on the number of days from today.
   * @property {Object[]} [presetRanges] - An array of preset date ranges with labels and start/end timestamps.
   * @property {boolean} [initialize=false] - Initialization flag to delay some actions.
   * @property {any} [tempEndDate=null] - Temporary end date for a date range.
   * @property {any} [prevStartDate=null] - Previous start date for a revert operation.
   * @property {any} [prevEndDate=null] - Previous end date for a revert operation.
   */
  ...
  let {
    ...
    startOfWeek = 0,
    isMultipane = $bindable(false),
    isRange = $bindable(false),
  ...
  const getDatesInRange = (startDate, endDate, disabled) => {
    ...
    for (; dateRangeStart <= dateRangeEnd; dateRangeStart.setDate(dateRangeStart.getDate() + 1)) {
      const formattedDate = `${dateRangeStart.getMonth() + 1}/${dateRangeStart.getDate()}/${dateRangeStart.getFullYear()}`;
      if (
        (!enabled && !disabled) ||
        (enabled.length && enabled.includes(Date.parse(formattedDate))) ||
  ...
  const onClick = function (e, day, month, year) {
    ...
    const event = {
      ...
      ...(isRange && {
        endDate,
        endDateTime,
        rangeDates: getDatesInRange(startDate, endDate, disabledDates),
      }),
    };
   ...
   *
   * @param {number} day - The day of the month.
   * @param {Array<Array<number>>} calendar - The calendar array.
  ...
  const onMouseEnter = function (e, day, month, year) {
    ...
    // @ts-ignore
    const { className: classes } = e.target || { className: undefined };
  ...
  $effect(() => {
    tempEndDate = tempEndDate ? getTimestamp(tempEndDate) : null;
  });
  ...
  $effect(() => {
    if (theme !== null) {
      globalThis.document?.documentElement.setAttribute("data-picker-theme", theme);
    }
  });
...
<div class="datepicker" data-picker-theme={theme} use:clickOutside={{ onClickOutside }}>
  ...
  >
    ...
    <div class="calendar">
      <header class:timepicker={showTimePicker}>
        ...
        <span>
          ...
          {#if showYearControls}
            <div class="years">
              <button type="button" onclick={toNextYear}>
                <i class="icon-next-year" aria-label="Next year" />
              </button>
              <button type="button" onclick={toPrevYear}>
                <i class="icon-previous-year" aria-label="Previous year" />
              </button>
            </div>
          {/if}
        </span>
        <button type="button" onclick={toNext} class:hide={!(!isRange || (isRange && !isMultipane))}>
      ...
      {#if showTimePicker && startDate}
        <div class="timepicker" class:show={isRange && !isMultipane}>
          ...
          {#if isRange}
            <input type="time" class="end-time" bind:value={endDateTime} oninput={() => (endDate = updateTime("end", endDate))} />
          {/if}
        </div>
      {/if}
            ...
            {#each { length: 7 } as d, dayIndex (dayIndex)}
              {#if startDateCalendar[weekIndex][dayIndex] !== 0}
                <button
                  ...
                  onmouseenter={(e) => onMouseEnter(e, startDateCalendar[weekIndex][dayIndex], startDateMonth, startDateYear)}
                  onmouseleave={onMouseLeave}
                  onclick={(e) => onClick(e, startDateCalendar[weekIndex][dayIndex], startDateMonth, startDateYear)}
    ...
    {#if isRange && isMultipane}
      <div class="calendar">
        <header class:timepicker={showTimePicker}>
          <button type="button" onclick={toPrev} class:hide={!(!isRange || (isRange && !isMultipane))}>
            <div class="icon-previous-month" aria-label="Previous month"></div>
          </button>
          <span>
              ...
              {#each { length: 7 } as d, dayIndex (dayIndex)}
                {#if endDateCalendar[weekIndex][dayIndex] !== 0}
                  <button
                    ...
                    onmouseenter={(e) => onMouseEnter(e, endDateCalendar[weekIndex][dayIndex], endDateMonth, endDateYear)}
                    onmouseleave={onMouseLeave}
                    onclick={(e) => onClick(e, endDateCalendar[weekIndex][dayIndex], endDateMonth, endDateYear)}
                    class:norange={isRange && tempEndDate === startDate}
                  >
